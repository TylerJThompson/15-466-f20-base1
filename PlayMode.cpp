...
bool PlayMode::handle_event(SDL_Event const &evt, glm::uvec2 const &window_size) {
	...
	if (evt.type == SDL_KEYDOWN) {
		if (evt.key.keysym.sym == SDLK_LEFT) {
			...
			if (!space.pressed && (player_at.y <= 0 || player_at.y >= 232)) {
...
void PlayMode::update(float elapsed) {
	...
	if (player_at.y > 0) {
		if (air_time_passed <= 0.75f) {
			player_at.y += PlayerSpeed * elapsed * 1.5f;
			air_time_passed += elapsed;
		}
		else if (air_time_passed <= 0.9f) air_time_passed += elapsed;
		else player_at.y -= PlayerSpeed * elapsed * 1.5f;
	}
	if (up.pressed && player_at.y <= 0) {
...
void PlayMode::draw(glm::uvec2 const &drawable_size) {
	...
	if (!(left.pressed || right.pressed)) {
		...
		ppu.sprites[0].index = 35 + gravity_sprite;
	}
	if (left.pressed) {
		if (walk_time_passed >= 0.25f) {
			if (ppu.sprites[0].index == (33 + gravity_sprite)) ppu.sprites[0].index = 35 + gravity_sprite;
			else ppu.sprites[0].index = 33 + gravity_sprite;
			walk_time_passed = 0.0f;
		}
	}
	if (right.pressed) {
		if (walk_time_passed >= 0.25f) {
			if (ppu.sprites[0].index == (34 + gravity_sprite)) ppu.sprites[0].index = 35 + gravity_sprite;
			else ppu.sprites[0].index = 34 + gravity_sprite;
			walk_time_passed = 0.0f;
		}
	}
	if (player_at.y > 0)
		ppu.sprites[0].index = 32 + gravity_sprite;
	...
	for (uint32_t i = 1; i < 63; ++i) {
		...
		if (i % 2) ppu.sprites[i].attributes |= 0b10000000; //'behind' bit
	}*/
